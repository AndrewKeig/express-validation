...
exports = module.exports = function (schema) {
  ...
  return function (req, res, next)  {
    ...
    ['headers', 'body', 'query', 'params', 'cookies'].forEach(function (key) {
      ...
      var stripUnknown = (key === 'body') ? options.stripUnknownBody : false;
      if (schema[key]) validate(errors, req, schema[key], key, allowUnknown, stripUnknown, entireContext);
    });
 ...
 */
function validate (errObj, request, schema, location, allowUnknown, stripUnknown, context) {
  if (!request[location] || !schema) return;
  ...
  Joi.validate(request[location], schema, joiOptions, function (errors, value) {
    if (!errors || errors.details.length === 0) {
      request[location] = value; // joi responses are parsed into JSON
      return;
    }
